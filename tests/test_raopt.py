...
class TestRuleBreakUpSelections(unittest.TestCase):
    ...
    def test_break_selections_project_select_gender_age_person(self):
        self._check(
            ...
            "\\project_{name}(\\select_{gender = 'f'} (\\select_{age = 16}"
            " Person));",
        )
    ...
    def test_break_selections_select_gender_age_person_cross_eats_flipped(
        ...
        self._check(
            "Eats \\cross \\select_{Person.gender='f' and"
            " Person.age=16}(Person);",
            "Eats \\cross \\select_{Person.gender = 'f'} (\\select_{Person.age"
            " = 16} Person);",
        )
    ...
    def test_select_rename_eats(self):
        self._check(
            ...
            " *}(Eats);",
            "\\select_{E.pizza = 'mushroom'} \\select_{E.price < 10}"
            " \\rename_{E: *}(Eats);",
        )
...
class TestRulePushDownSelections(unittest.TestCase):
    ...
    def test_select_pizza_select_gender_person_cross_eats(self):
        self._check(
            "\\select_{pizza = 'mushroom'} \\select_{gender = 'm'} (Person"
            " \\cross Eats);",
            "(\\select_{gender = 'm'} Person) \\cross (\\select_{pizza ="
            " 'mushroom'} Eats);",
        )
    ...
    def test_select_select_person_cross_eats(self):
        self._check(
            "\\select_{age = 16} \\select_{Person.name = Eats.name} (Person"
            " \\cross Eats);",
    ...
    def test_select_select_cross_2rename(self):
        self._check(
            "\\select_{Eats1.pizza = Eats2.pizza} \\select_{Eats1.name ="
            " 'Amy'} (\\rename_{Eats1: *}(Eats) \\cross \\rename_{Eats2:"
            " *}(Eats));",
            "\\select_{Eats1.pizza = Eats2.pizza} ((\\select_{Eats1.name ="
            " 'Amy'} \\rename_{Eats1: *}(Eats)) \\cross \\rename_{Eats2:"
            " *}(Eats));",
    ...
    def test_select_select_cross_2rename_flipped(self):
        self._check(
            "\\select_{Eats1.pizza = Eats2.pizza} \\select_{Eats2.name ="
            " 'Amy'} (\\rename_{Eats1: *}(Eats) \\cross \\rename_{Eats2:"
            " *}(Eats));",
            "\\select_{Eats1.pizza = Eats2.pizza} (\\rename_{Eats1: *}(Eats)"
            " \\cross (\\select_{Eats2.name = 'Amy'} \\rename_{Eats2:"
            " *}(Eats)));",
        )

    def test_select_person_cross_select_eats(self):
        self._check(
            "(\\select_{Person.name = 'Amy'} Person) \\cross"
...
class TestMergeSelections(unittest.TestCase):
    ...
    def test_cross_select_select(self):
        self._check(
            "Pizzeria \\cross (\\select_{pizza = 'mushroom'} \\select_{price ="
            " 10} Serves);",
            "Pizzeria \\cross (\\select_{pizza = 'mushroom' and price = 10}"
            " Serves);",
        )

    def test_select_select_select_person(self):
        self._check(
            "\\select_{name = 'Amy'} \\select_{gender = 'f'} \\select_{age ="
            " 16} Person;",
...
class TestIntroduceJoins(unittest.TestCase):
    ...
    def test_select_person_cross_select_eats(self):
        self._check(
            ...
            " \\select_{Person.name = 'Amy'} Eats);",
            "Person \\join_{Person.name = Eats.name} \\select_{Person.name ="
            " 'Amy'} Eats;",
        )
...
class TestAllSteps(unittest.TestCase):
    ...
    def test_project_join2(self):
        self._check(
            "\\select_{Person.name = Eats.name and Person.name = Eats.pizza"
            " and Eats.name = 'Amy'} (Person \\cross Eats);",
            "Person \\join_{Person.name = Eats.name and Person.name ="
    ...
    def test_cross_cross(self):
        self._check(
            ...
            " Person.name = Eats.name} ((Person \\cross Eats) \\cross"
            " Serves);",
            "\\project_{Person.name} ((Person \\join_{Person.name = Eats.name}"
    ...
    def test_renamings(self):
        self._check(
            ...
            "\\project_{P.name, E.pizza} ((\\rename_{P: *} Person)"
