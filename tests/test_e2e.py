...
class End2EndUnitTests(unittest.TestCase):
    ...
    def test_project_person_join_eats_join_serves(self):
        sqlstring = (
            "select distinct Person.name, Serves.pizza from Person, Eats,"
            " Serves where Person.name = Eats.name and Eats.pizza ="
            " Serves.pizza"
        )
        computed = self._evaluate(sqlstring)
        self.assertEqual(len(computed), 20)
    ...
    def test_mushroom_lovers(self):
        sqlstring = (
            "select distinct Person.name, Serves.pizzeria from Person, Eats,"
            " Serves where Person.name = Eats.name and Eats.pizza ="
            " Serves.pizza and Eats.pizza = 'mushroom'"
        )
        computed = self._evaluate(sqlstring)
    ...
    def test_mushroom_lovers_renaming(self):
        sqlstring = (
            "select distinct P.name, S.pizzeria from Person P, Eats E, Serves"
            " S where P.name = E.name and E.pizza = S.pizza and E.pizza ="
            " 'mushroom'"
        )
        computed = self._evaluate(sqlstring)
        self.assertEqual(len(computed), 8)
