...
class SQL2RATests(TestSQL2RA):
    ...
    def test_select_star_from_person_where_age(self):
        self._check(
            "select distinct * from Person where age=16", "\select_{age=16}(Person);"
        )
    ...
    def test_select_person_name_from_person(self):
        self._check(
            "select distinct Person.name from Person", "\project_{Person.name}(Person);"
        )

    def test_select_name_age_from_person(self):
        self._check(
            "select distinct name, age from Person", "\project_{name, age}(Person);"
        )
